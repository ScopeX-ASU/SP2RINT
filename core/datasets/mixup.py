...
class Mixup:
    ...
    def _params_per_batch(self):
        ...
        if self.mixup_enabled and np.random.rand() < self.mix_prob:
            ...
            lam = float(lam_mix)
            phase = np.exp(
                1j * np.random.beta(self.mixup_alpha, self.mixup_alpha) * 2 * np.pi
            ).item()
        return lam, phase, use_cutmix

    def _mix_elem(self, x):
        batch_size = len(x)
        lam_batch, phase_batch, use_cutmix = self._params_per_elem(
            batch_size * x.size(1)
        )
...
def mixup_target(target, lam, phase, indices):
    return target * lam + target.flatten(0, 1)[indices].contiguous().view_as(
        target
    ).mul((1 - lam) * phase)
...
class MixupAll:
    ...
    def _params_per_elem(self, batch_size, n_ports, device, random_state=None):
        ...
        I = np.eye(n_ports, dtype=np.complex64)[
            np.newaxis, ...
        ]  # [1,mode, mode]  # noqa: E741

        if self.mixup_enabled:
            lam /= np.sum(lam**2, axis=1, keepdims=True)  # normalize total energy
            phase = np.random.uniform(
                0, 2 * np.pi, size=[batch_size, n_ports, n_ports]
            ).astype(np.float32)  # random phase shift
    ...
    def _mix_elem(self, x, random_state=None):
        batch_size = len(x)
        lam_batch = self._params_per_elem(
            batch_size, x.shape[1], x.device, random_state
        )  # [bs, mode, mode] complex
        # x [bs, mode, inc, h, w] complex
