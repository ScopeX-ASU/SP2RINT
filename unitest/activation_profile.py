...
def main() -> None:
    ...
    try:
        if (
            ...
            and len(configs.checkpoint.test_restore_checkpoint) > 0
        ):
            load_model(
                model,
            ...
            load_model(
                model_test,
                configs.checkpoint.test_restore_checkpoint,
                ignore_size_mismatch=int(configs.checkpoint.no_linear),
            )
            ...
            x = x.squeeze(1)
            x1 = torch.matmul(x, near2far_matrix.T)
            x2 = torch.matmul(x1, (near2far_matrix@model_test.state_dict()[f"features.conv1.conv._conv_pos.metalens.0_17.W_buffer"]).T)
            x3 = torch.matmul(x2, (near2far_matrix@model_test.state_dict()[f"features.conv1.conv._conv_pos.metalens.1_17.W_buffer"]).T)

            x1_fft = torch.fft.fft(x1, dim=-1).abs().square().mean(dim=0)
            x1_fft = x1_fft / torch.max(x1_fft)
            x2_fft = torch.fft.fft(x2, dim=-1).abs().square().mean(dim=0)
            x2_fft = x2_fft / torch.max(x2_fft)
            x3_fft = torch.fft.fft(x3, dim=-1).abs().square().mean(dim=0)
            x3_fft = x3_fft / torch.max(x3_fft)

            with h5py.File(f"./core/fft_weights.h5", "w") as f:
                f.create_dataset("first_layer", data=x1_fft.cpu().numpy())
                f.create_dataset("second_layer", data=x2_fft.cpu().numpy())
