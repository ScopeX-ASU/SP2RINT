...
class N_Ports(BaseDevice):
    ...
    def add_monitor_slice(
        ...
        if direction[0] == "x":
            ...
            # if slice_name == "nearfield_1":
            #     print(f"this is how monitor half width are calculated: {size[1]} / {2} / {self.grid_step} = {size[1] / 2 / self.grid_step}", flush=True)
            #     print("this is the begin of y: ", max(0, monitor_center[1] - monitor_half_width), flush=True)
            #     print("this is the end of y: ", min(self.Ny, monitor_center[1] + monitor_half_width), flush=True)
            #     print("this is the monitor center: ", monitor_center, flush=True)
            #     print("this is the monitor half width: ", monitor_half_width, flush=True)
        elif direction[0] == "y":
            monitor_center = [
                int(round((c + offset / 2) / self.grid_step))
                for c, offset in zip(center, self.cell_size)
