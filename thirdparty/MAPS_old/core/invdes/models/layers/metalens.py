...
class MetaLens(N_Ports):
    def __init__(
        ...
        elif design_var_type == "height":
            # make sure that the aperture can be devided by atom_width perfectly
            assert round(aperture * 10) % round(atom_period * 10) == 0, f"The aperture should be divided by atom_period perfectly! but got {aperture} % {atom_period} = {aperture % atom_period}"
            num_atoms = round(aperture / atom_period)
            for i in range(num_atoms):
                design_region_cfgs[f"design_region_{i}"] = dict(
                    ...
                    center=[
                        ...
                        + substrate_depth
                        + ridge_height_max / 2,
                        -aperture / 2 + atom_period / 2 + i * atom_period,
                    ],
                    size=[ridge_height_max, atom_width],
                    eps=eps_r_fn(wl_cen),
                    eps_bg=eps_bg_fn(wl_cen),
                )
        else:
            raise ValueError(
                f"Unknown design_var_type: {design_var_type}. Only support width or height."
            )
    ...
    def init_monitors(self, verbose: bool = True):
        ...
        #     center=(
        #         self.farfield_dxs[0][1] # hard code for now
        #         + self.port_cfgs["out_port_1"]["center"][0]
        #         - self.port_cfgs["out_port_1"]["size"][0] / 2,
        #         self.nearfield_offset,
        #     ),
