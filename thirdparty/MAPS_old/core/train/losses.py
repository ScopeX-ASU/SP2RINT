...
class DarcyLoss(torch.nn.modules.loss._Loss):
    def __init__(self, p=1, size_average=True, reduce=None, reduction=True) -> None:
        super().__init__(size_average, reduce, reduction)
        self.size_average = size_average
        self.p = p

    def forward(self, u, a):
        # u is predicted flow, a is input
        batchsize = u.size(0)
        size = u.size(-1)
        u = u.reshape(batchsize, size, size)
        a = a.reshape(batchsize, size, size)
        lploss = LpLoss(
            p=self.p, size_average=self.size_average, reduction=self.reduction
        )
