...
from autograd.extend import defjvp, defvjp, primitive, vspace
...
def make_sparse(entries, indices, shape):
    ...
    """
    coo = sp.coo_matrix((entries, indices), shape=shape, dtype=entries.dtype)
    return coo.tocsr()
...
def reshape_to_ND(arr, N):
    ...
    if ND > N:
        raise ValueError(
            "array is larger than {} dimensional, given shape {}".format(N, arr.shape)
        )
    extra_dims = (N - ND) * (1,)
...
def aniplot(F, source, steps, component="Ez", num_panels=10):
    ...
    for t_index in range(steps):
        ...
        if t_index % (steps // num_panels) == 0:
            if (
                ax_index < num_panels
            ):  # extra safety..sometimes tries to access num_panels-th elemet of ax_list, leading to error
                print(
                    "working on axis {}/{} for time step {}".format(
                        ax_index, num_panels, t_index
                    )
                )
...
from autograd.extend import defjvp, primitive
